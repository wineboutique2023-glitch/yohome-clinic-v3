import { useEffect, useRef, useState } from "react";import { supabase } from "./lib/supabase";import "./styles.css";

const emptyClient = {first_name: "",last_name: "",phone: "",email: "",date_of_birth: "",suburb_area: "",emergency_contact: "",notes: "",};

const emptyIntake = {main_concern: "",pain_area: "",pain_level: "",medical_history: "",medications: "",allergies: "",contraindications: "",consent_given: false,signature_name: "",signature_date: "",signature_image: "",body_chart_notes: "",body_chart_image: "",risk_alerts: "",};

const emptySoap = {treatment_date: "",therapist_name: "",subjective: "",objective: "",assessment: "",plan: "",therapist_notes: "",};

export default function App() {const [clients, setClients] = useState([]);const [search, setSearch] = useState("");const [selectedClient, setSelectedClient] = useState(null);const [clientForm, setClientForm] = useState(emptyClient);const [intakeForm, setIntakeForm] = useState(emptyIntake);const [intakeHistory, setIntakeHistory] = useState([]);const [soapForm, setSoapForm] = useState(emptySoap);const [soapNotes, setSoapNotes] = useState([]);const [activeTab, setActiveTab] = useState("client");const [loading, setLoading] = useState(false);const [therapists, setTherapists] = useState([]);const [newTherapist, setNewTherapist] = useState("");

const signatureRef = useRef(null);const bodyChartRef = useRef(null);const isSigningRef = useRef(false);const isDrawingBodyRef = useRef(false);

useEffect(() => {fetchClients();fetchTherapists();}, []);

useEffect(() => {if (activeTab !== "intake") return;
...
async function fetchTherapists() {const { data, error } = await supabase.from("therapists").select("*").order("name", { ascending: true });
...
async function addTherapist() {const name = newTherapist.trim();if (!name) return;
...
async function deleteTherapist(id) {if (!window.confirm("Delete this therapist?")) return;
...
async function fetchClients() {const { data, error } = await supabase.from("clients").select("*").order("created_at", { ascending: false });
...
async function selectClient(client) {setSelectedClient(client);setClientForm(client);setActiveTab("client");await fetchIntake(client.id);await fetchIntakeHistory(client.id);await fetchSoapNotes(client.id);}

async function fetchIntake(clientId) {// Safe version:// Supabase .single() / .maybeSingle() will break if the same client accidentally has// more than one intake record. This always takes the latest one instead.const { data, error } = await supabase.from("intake_forms").select("*").eq("client_id", clientId).order("created_at", { ascending: false }).limit(1);
...
async function fetchIntakeHistory(clientId) {const { data, error } = await supabase.from("intake_forms").select("*").eq("client_id", clientId).order("created_at", { ascending: false });
...
async function fetchSoapNotes(clientId) {const { data, error } = await supabase.from("soap_notes").select("*").eq("client_id", clientId).order("created_at", { ascending: false });
...
function handleClientChange(e) {setClientForm({ ...clientForm, [e.target.name]: e.target.value });}

function handleIntakeChange(e) {const { name, value, type, checked } = e.target;setIntakeForm({...intakeForm,[name]: type === "checkbox" ? checked : value,});}

function handleSoapChange(e) {setSoapForm({ ...soapForm, [e.target.name]: e.target.value });}

async function saveNewClient() {if (!clientForm.first_name && !clientForm.last_name) {return alert("Please enter client name.");}
...
async function updateClient() {if (!selectedClient) return alert("Please select a client first.");
...
async function deleteClient() {if (!selectedClient) return alert("Please select a client first.");
...
async function saveIntake() {if (!selectedClient) return alert("Please select a client first.");
...
async function saveSoapNote() {if (!selectedClient) return alert("Please select a client first.");
...
async function deleteSoapNote(id) {if (!window.confirm("Delete this SOAP note?")) return;
...
function safeText(value) {return value || "Not recorded";}

function loadIntakeFromHistory(record) {setIntakeForm({...emptyIntake,...(record || {}),});setTimeout(() => {restoreSignatureCanvas();restoreBodyChartCanvas();}, 100);}

function cleanFileName(value) {return String(value || "").trim().replace(/[^a-zA-Z0-9-_ ]/g, "").replace(/\s+/g, "").replace(/+/g, "_");}

function getClientFileName(label) {const today = new Date().toISOString().slice(0, 10);const firstName = selectedClient?.first_name || "Client";const lastName = selectedClient?.last_name || "";const clientName = cleanFileName(${firstName} ${lastName}) || "Client";return YOHOME_${clientName}_${today}_${label};}

function getCanvasPoint(canvas, event) {const rect = canvas.getBoundingClientRect();const clientX = event.touches ? event.touches[0].clientX : event.clientX;const clientY = event.touches ? event.touches[0].clientY : event.clientY;
...
function startSignature(e) {e.preventDefault();const canvas = signatureRef.current;if (!canvas) return;
...
function drawSignature(e) {e.preventDefault();if (!isSigningRef.current) return;
...
function stopSignature() {isSigningRef.current = false;
...
function clearSignature() {const canvas = signatureRef.current;if (!canvas) return;
...
function restoreSignatureCanvas() {const canvas = signatureRef.current;if (!canvas) return;
...
function openPdfWindow(html) {const win = window.open("", "_blank");win.document.write(html);win.document.close();}

function downloadSoapPdf() {if (!selectedClient) return alert("Please select a client first.");
...
function downloadFullReportPdf() {if (!selectedClient) return alert("Please select a client first.");
...
function newClient() {setSelectedClient(null);setClientForm(emptyClient);setIntakeForm(emptyIntake);setIntakeHistory([]);setSoapForm(emptySoap);setSoapNotes([]);setActiveTab("client");}

const filteredClients = clients.filter((client) => {const text = ${client.first_name || ""} ${client.last_name || ""} ${
      client.phone || ""
...
return (<div className="app"><aside className="sidebar"><div className="brand"><h2>YOHOME</h2><p>Client Records</p></div>
